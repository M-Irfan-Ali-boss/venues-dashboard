...
const submit = () => {
  form.post(route('update-venue'), {
    onFinish: () => {
      form.reset();
    },
    onSuccess: () => toast.success('Venue has been updated.'),
  });
};
...
watch(
  ...
  (newVenue) => {
    ...
    form.phone = newVenue?.phone;
    form.address = newVenue?.address;
    form.website = newVenue?.website;
    form.description = newVenue?.description;
    form.status = newVenue?.status;
    form.type = newVenue?.type;
    form.latitude = newVenue?.latitude;
    form.longitude = newVenue?.longitude;
    form.staff_id = newVenue?.staff_id;
  },
  { immediate: true }
);
</script>
